...
public class AadharClient {
     ...
     */
    @Retry(name = "aadhar", fallbackMethod = "verifyAadharFallback")
    public boolean verifyAadhar(String aadharNumber) {
        ...
        try {
            ...
            // MOD11-BUG-01 FIXED: surface the real 4xx validation message instead of a generic outage
            log.error("Aadhar validation error ({}): {}", e.getStatusCode(), e.getResponseBodyAsString());
    ...
    /**
     * MOD11-CR-01: fallback invoked when all retry attempts are exhausted.
     * Rethrows so a real validation message (4xx) still reaches the user.
     */
    public boolean verifyAadharFallback(String aadharNumber, Throwable t) {
        log.error("Aadhar verification failed after retries: {}", t.getMessage());
